...
public class JsonToIfcV2 {
    ...
    public JsonToIfcV2(String jsonFile) throws IOException {
    ...
    private void fillEntity(LinkedHashMap<String, Object> attributeData) {
        ...
        attributeData.forEach((key, value) -> {
            ...
            List<Method> collect = methods.stream().filter(v -> v.getName().equals("set" + StringUtils.capitalize(String.valueOf(key)))).collect(Collectors.toList());
            ...
            try {
                if (StringConstant.GLOBAL_ID.equalsIgnoreCase(key)) {
                    targetMethod.invoke(ifcObject, new IfcGloballyUniqueId(GlobalUniqueID.compress(GlobalUniqueID.merge(String.valueOf(value)))));
                    return;
                }
            ...
            synchronized (this.singletonObjects) {
                ...
                if (null == ifcObject) {
                    ...
                    try {
                        ...
                        throw new SikongSphereParseException(String.format("Class for %s does not instantiated successfully.", className));
                    }
    ...
    private IfcAbstractClass dealWithMap(LinkedHashMap<String, Object> value) throws InvocationTargetException, IllegalAccessException, InstantiationException, NoSuchMethodException {
        ...
        value.forEach((propertyName, propertyValue) -> {
            ...
            List<Method> targetMethods = methods.stream().filter(method -> method.getName().equals("set" + StringUtils.capitalize(propertyName))).collect(Collectors.toList());
            try {
                if (StringConstant.GLOBAL_ID.equalsIgnoreCase(propertyName)) {
                    // globalId
                    targetMethods.get(0).invoke(ifcObject, new IfcGloballyUniqueId(GlobalUniqueID.compress(GlobalUniqueID.merge(String.valueOf(value)))));
                    ...
                    // string
                    targetMethods.get(0).invoke(ifcObject, this.dealWithString(String.valueOf(propertyValue), targetMethods.get(0)));
                } else if (LinkedHashMap.class.isAssignableFrom(propertyValue.getClass())) {
                    // Map
                    targetMethods.get(0).invoke(ifcObject, this.dealWithMap((LinkedHashMap<String, Object>) propertyValue));
                    ...
                    for (Method targetMethod : targetMethods) {
                        ...
                        if (targetMethod.getParameterTypes()[0].isAssignableFrom(o.getClass())) {
...
//        method.get
    ...
    public static void main(String[] args) throws IOException, InstantiationException,
        ...
        JsonToIfcV2 jsonToIfc = new JsonToIfcV2("src/test/resources/blank_sikongsphere.json");
